...
# upload to master
rsync -avz evaluation.py hotel-reservation requirements.txt setup-node.sh social-network traces train-ticket utils.py root@autothrottle-1:
...
mkdir -p tmp
rsync -avz root@autothrottle-1:join-command tmp/
rsync -avz root@autothrottle-1:kube-config tmp/
...
# Setup SSH
# for i in {1..5}; do
#     ssh root@autothrottle-$i "ssh-keygen -t rsa -N '' -f /root/.ssh/id_rsa <<<y ; cat /root/.ssh/id_rsa.pub"
# done
